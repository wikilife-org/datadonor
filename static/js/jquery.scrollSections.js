    ...
    Plugin.prototype = {
        ...
        touch: function () {
            ...
            this._$body.bind('touchstart', function (event) {
                ...
                self._$body.bind('touchmove', function (event) {
                    var diff = {
                        x: startEvent.clientX - event.clientX,
                        y: startEvent.clientY - event.clientY
                    };
                    var nextStep;
                    event.preventDefault();
        ...
        customScrollTo: function (index, noAnimation) {
            ...
            if (index != null && index >= 0 && index < this._sections) {
                ...
                $.when(this._$htmlBody.stop(true, false) .animate({
                    scrollTop: yTo
                }, speed)
                ) .done(function () {
        ...
        mousewheel: function () {
            ...
            this._$window.mousewheel(function (event, delta, deltaX, deltaY) {
                ...
                if (!(self._isAnimated && self._scrollPaused)) {
                    deltaY = deltaY >> 0;
                    // Because steps numbers are integers
                    // Scroll Down
                    if (deltaY < 0) {
        ...
        init: function () {
            ...
            if (this._sections > 0) {
                ...
                if ((new RegExp('MSIE ([0-9]{1,}[\.0-9]{0,})')) .exec(window.navigator.userAgent) && (parseFloat(RegExp.$1) <= 8)) {
                    this._ltIE9 = true;
                }
