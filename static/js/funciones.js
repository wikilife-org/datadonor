...
$(document) .ready(function () {
    ...
    function $nextNetwork() {
        ...
        if (overflowWidth < visibleArea) {
            ...
            $nuevaPosicion.animate({
                left: - hiddenOverflow + 'px'
            }, 400);
            $nextCarousel.hide();
        } else {
            $nuevaPosicion.stop(true, true);
    ...
    $('.social_icons .boton') .click(function (event) {
        event.preventDefault();
    })
    ...
    $('ul.ui-autocomplete') .hover(function () {
    ...
    // AUTOCOMPLETES TRIGGER
    // STEP 16
    $('#mood_1 a') .on('click', function (event) {
        event.preventDefault();
    });
    $('#mood_2 a') .on('click', function (event) {
        event.preventDefault();
    });
    // sliders your
    $('#mood_1') .slider({
